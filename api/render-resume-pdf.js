...
const renderResumePdf = async ({ html, filenameBase }) => {
...
export default async function handler(request, response) {
  ...
  if (typeof html !== 'string' || !html.trim()) {
    return json(response, 400, { error: 'HTML content is required.' });
  }

  try {
    const { filename, pdfBuffer } = await renderResumePdf({
      html,
      filenameBase,
    });
    ...
    response.setHeader('Content-Length', String(binaryPdfBuffer.byteLength));
    response.setHeader('Cache-Control', 'no-store');
    response.send(binaryPdfBuffer);
  } catch (error) {
    const details = error instanceof Error ? error.message : 'Failed to render PDF.';
    console.error('Vercel PDF render error:', details);
